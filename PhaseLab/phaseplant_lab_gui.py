...
class PhasePlantLabGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        self.random_use_full_folder = tk.BooleanVar(value=False)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(text_wrap, text="Preset Diffusion + Fusion + Catalyze Toolkit", style="SubHeader.TLabel").pack(anchor="w")
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Catalyze: independent mode or full-folder mode."
            ),
            foreground="#555",
            wraplength=300,
    ...
    def _build_mutate_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(parent, text="Create Diffused Preset", command=self.run_mutate).grid(
    ...
    def _build_combine_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(parent, text="Create Fused Preset", command=self.run_combine).grid(
    ...
    def _build_random_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(
            parent,
            text="Catalyze generates new presets from learned valid structures.",
            foreground="#8b9bb2",
        ).grid(row=0, column=0, columnspan=2, sticky="w", pady=6)
        ...
        ttk.Checkbutton(parent, text="Use Full Folder As Source Pool", variable=self.random_use_full_folder).grid(
            row=4, column=0, columnspan=2, sticky="w", pady=(2, 2)
        )
        ttk.Label(
            parent,
        ...
        ttk.Button(parent, text="Create Catalyzed Preset", command=self.run_random).grid(
            row=6, column=0, columnspan=2, sticky="w", pady=(14, 0)
        )
        ttk.Label(parent, text="Recent Output", foreground="#8aa0bf").grid(
            row=7, column=0, columnspan=2, sticky="w", pady=(14, 4)
        )
        ttk.Label(
            parent,
            textvariable=self.output_log_var,
            foreground="#dbe8ff",
            wraplength=620,
            justify="left",
    ...
    def _is_exact_factory_proxy_match(self, factory_path: Path, preset: object) -> bool:
        ...
        if source is None:
            return False
        return self._normalized_stem(str(source)) == self._normalized_stem(factory_path.name)

    def _pick_exact_factory_proxy(self, factory_path: Path, pool: list) -> object | None:
        exact = [preset for preset in pool if self._is_exact_factory_proxy_match(factory_path, preset)]
        if not exact:
            return None
        nongenerated = [preset for preset in exact if not self._is_phaselab_generated_preset(preset)]
        if nongenerated:
            exact = nongenerated
        else:
        ...
        def rank(preset: object) -> tuple[int, int]:
            ...
            return converted_rank, len(source_path)
    ...
    def _build_factory_proxy_pool(
        ...
        if len(editable_paths) > max_candidates:
            wanted_stems = {self._normalized_stem(path.name) for path in factory_paths}
            exact_name_paths = [path for path in editable_paths if self._normalized_stem(path.name) in wanted_stems]
            exact_keys = {str(path.resolve()) for path in exact_name_paths}
            remainder = [path for path in editable_paths if str(path.resolve()) not in exact_keys]
            keep = exact_name_paths[:max_candidates]
        ...
        for factory_path in factory_paths:
            stem = self._normalized_stem(factory_path.name)
            candidates = [
                preset
                for key, key_presets in presets_by_stem.items()
                if self._normalized_stem(key) == stem
        ...
        if direct_matches:
            ...
            note = (
                f"Proxy synthesis used {len(direct_matches)} exact editable match(es) by factory preset name "
    ...
    def run_mutate(self) -> None:
        try:
            ...
            if self._is_zip_preset(source_path):
                ...
                has_exact_proxy = base_preset is not None
                if has_exact_proxy and base_preset is not None:
                    ...
                    summary += f" Exact editable base: {Path(base_preset.source).name}."
                    ...
                    if not has_exact_proxy:
                        summary += (
                            " No exact converted/editable match found for this factory preset; "
        ...
        try:
            ...
            if binary_paths:
                ...
                for path in binary_paths:
                    picked = self._pick_exact_factory_proxy(path, proxy_pool)
                    if picked is None:
                        exact_proxy_pool = []
                        break
                    if all(id(existing) != id(picked) for existing in exact_proxy_pool):
                        exact_proxy_pool.append(picked)
                if len(exact_proxy_pool) >= 2 and len(exact_proxy_pool) == len(binary_paths):
                    base_state = exact_proxy_pool[0].state
                    donor_states = [p.state for p in exact_proxy_pool[1:]]
                    enum_pool = build_enum_pool([p.state for p in exact_proxy_pool])
                    assets, warnings = merge_assets(exact_proxy_pool)
                    ...
                    effective_mix = max(0.45, mix_rate)
                    effective_mut = max(0.20, mutate_amount)
                    for _ in range(count):
                    ...
                    summary += " Used exact converted/editable matches for every factory source."
                    summary += f" {proxy_note}"
                    if use_full_folder:
    ...
    def run_random(self) -> None:
        try:
            ...
            output_dir = self.get_output_dir()
            rng = random.Random(seed)
            use_full_folder = bool(self.random_use_full_folder.get())

            if use_full_folder:
                preset_dir = self.get_preset_dir()
                if not preset_dir.is_dir():
                    raise ValueError("Preset folder not found. Select a valid preset folder.")
                all_paths = list(self.preset_paths)
                if not all_paths:
                    raise ValueError("Random generation needs at least 1 preset in the selected folder")
                editable_paths = [path for path in all_paths if self._is_zip_preset(path)]
                binary_paths = [path for path in all_paths if not self._is_zip_preset(path)]

                if editable_paths:
                    editable_presets, skipped_invalid = self._load_valid_editable_presets(editable_paths)
                    if editable_presets:
                        ...
                        summary += f". Full-folder source mode ({len(editable_presets)} valid editable source(s))."
                        if skipped_invalid:
                            summary += f". Skipped invalid sources: {len(skipped_invalid)}"
                        if total_repairs:
                            summary += f". Integrity fixes applied: {total_repairs}"
                        ...
                        if binary_paths:
                            summary += f". Skipped factory/binary sources: {len(binary_paths)}"
                    else:
                        editable_paths = []
                ...
                if not editable_paths:
                    if binary_paths:
                        proxy_pool, proxy_note = self._build_factory_proxy_pool(
                            binary_paths,
                            rng,
                            max_pool=max(8, min(24, len(binary_paths) * 2)),
                        )
                        if proxy_pool:
                            outputs, total_generated_changes, total_repairs, warning_count = self._generate_random_from_editable_pool(
                                proxy_pool,
                            ...
                            self.refresh_preset_list()
                            summary = (
                                f"Created {len(outputs)} factory-context catalyzed preset(s) via proxy synthesis "
                                f"(total generated parameter changes: {total_generated_changes}, complexity: {complexity:.2f}). "
                                f"{self.summarize_outputs(outputs)}"
                            )
                            summary += f" {proxy_note}"
                        ...
                        global_paths = self._discover_global_editable_paths()
                        global_presets, global_skipped = self._load_valid_editable_presets(global_paths)
                        if global_presets:
                            outputs, total_generated_changes, total_repairs, warning_count = self._generate_random_from_editable_pool(
                                global_presets,
                                rng,
                            ...
                            summary += ". Folder had no valid editable sources, so PhaseLab used independent generation mode."
                            if global_skipped:
                                summary += f" Skipped invalid independent sources: {len(global_skipped)}."
                            if total_repairs:
                                summary += f" Integrity fixes applied: {total_repairs}."
                            if warning_count:
                                summary += f" Asset warnings: {warning_count}."
                        else:
                            raise ValueError("Need at least 1 valid source preset in the selected folder")
            else:
                global_paths = self._discover_global_editable_paths()
                global_presets, global_skipped = self._load_valid_editable_presets(global_paths)
                if not global_presets:
                    raise ValueError(
                        "Independent catalyze mode needs at least 1 valid editable preset in your user preset library."
                    )
                outputs, total_generated_changes, total_repairs, warning_count = self._generate_random_from_editable_pool(
                    global_presets,
                    rng,
                ...
                self.refresh_preset_list()
                summary = (
                    f"Created {len(outputs)} independent catalyzed preset(s) "
                    f"(total generated parameter changes: {total_generated_changes}, complexity: {complexity:.2f}). "
                    f"{self.summarize_outputs(outputs)}"
                )
                if global_skipped:
