...
USER_PRESETS_DIR = Path.home() / "Library/Audio/Presets/Kilohearts/Phase Plant/User Presets"
SYSTEM_FACTORY_PRESET_ROOT = Path("/Library/Application Support/Kilohearts/presets/kphp/Factory Presets")
PREFERRED_CONVERTED_FACTORY_DIRS = (
    "factory copy",
    "converted factory presets",
    "converted factory banks and presets",
    "converted presets from converter",
    "converted factory proxies",
)
STYLE_OPTIONS = ("Auto", "Bass", "Lead", "Pad", "Pluck", "Keys", "Drums", "FX", "Sequence")
STYLE_KEYWORDS = {
    "bass": {"bass", "sub", "reese", "wobble", "growl", "dnb", "neuro", "808"},
    "lead": {"lead", "supersaw", "solo", "mono", "hook"},
    "pad": {"pad", "ambient", "choir", "string", "atmo", "drone"},
    "pluck": {"pluck", "kalimba", "harp", "mallet"},
    "keys": {"keys", "piano", "epiano", "rhodes", "organ", "bell", "clav"},
    "drums": {"drum", "kick", "snare", "hat", "hihat", "clap", "perc", "tom", "rim", "cymbal"},
    "fx": {"fx", "impact", "riser", "sweep", "noise", "whoosh", "transition"},
    "sequence": {"seq", "sequence", "arp", "arpeggio", "trance", "pattern", "pulse"},
}
...
class PhasePlantLabGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        self.mutate_count = tk.IntVar(value=1)
        ...
        self.combine_count = tk.IntVar(value=1)
        ...
        self.random_count = tk.IntVar(value=1)
        self.random_use_guidance = tk.BooleanVar(value=True)
        self.random_style = tk.StringVar(value="Auto")

        self.anomalize_count = tk.IntVar(value=1)
        ...
        self._factory_proxy_index_roots: tuple[str, ...] = ()
        self._factory_proxy_rel_index: dict[str, Path] = {}
        self._factory_proxy_name_index: dict[str, list[Path]] = {}
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(text_wrap, text="Phase Plant preset Diffusion + Fusion + Catalyze + Anomalize Toolkit", style="SubHeader.TLabel").pack(anchor="w")
    ...
    def _add_slider(
        ...
        *,
        ...
        scale = ttk.Scale(holder, from_=from_value, to=to_value, variable=variable, orient="horizontal", length=260)
        scale.pack(side="left")
        label_width = 4 if integer else 5
        value_label = ttk.Label(holder, width=label_width)
        ...
        def sync_label(*_: object) -> None:
            if integer:
                variable.set(int(round(float(variable.get()))))
            value_label.configure(text=format_value())

        variable.trace_add("write", sync_label)
        scale.configure(command=lambda _: sync_label())
        self._bind_scale_click_jump(scale, variable, from_value, to_value, integer)
        sync_label()

    def _bind_scale_click_jump(
        self,
        scale: ttk.Scale,
        variable: tk.Variable,
        from_value: float,
        ...
        def set_from_event(event: tk.Event) -> str:
            ...
            x = min(max(float(getattr(event, "x", 0)), 0.0), float(width))
            ratio = x / float(width)
    ...
    def _build_random_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(parent, text="Style Focus").grid(row=4, column=0, sticky="w", pady=6)
        ttk.Combobox(
            parent,
            textvariable=self.random_style,
            state="readonly",
    ...
    def parse_count(self, raw: object, field_name: str) -> int:
        value = int(str(raw).strip())
        if value < 1:
            raise ValueError(f"{field_name} must be >= 1")
        if value > 20:
            raise ValueError(f"{field_name} must be <= 20")
        return value
    ...
    def _discover_global_editable_paths(self, limit: int = 1200) -> list[Path]:
        roots: list[Path] = []
        roots.extend(self._known_converted_factory_roots())
        roots.extend([APP_DIR.parent, USER_PRESETS_DIR, self.get_preset_dir()])
        paths: list[Path] = []
        seen: set[str] = set()
        for root in roots:
            if not root.is_dir():
    ...
    def _known_converted_factory_roots(self) -> list[Path]:
        candidates = [
            APP_DIR.parent / name for name in PREFERRED_CONVERTED_FACTORY_DIRS
        ] + [
            USER_PRESETS_DIR / name for name in PREFERRED_CONVERTED_FACTORY_DIRS
        ]
        roots: list[Path] = []
        seen: set[str] = set()
    ...
    def _refresh_factory_proxy_index(self, force: bool = False) -> None:
        roots = tuple(str(path.resolve()) for path in self._known_converted_factory_roots())
        if not force and roots == self._factory_proxy_index_roots and self._factory_proxy_name_index:
            return

        rel_index: dict[str, Path] = {}
        name_index: dict[str, list[Path]] = {}
        for root in self._known_converted_factory_roots():
            for path in root.rglob("*.phaseplant"):
                if not path.is_file() or not self._is_zip_preset(path):
                    continue
                rel_key = str(path.relative_to(root).as_posix()).lower()
                rel_index.setdefault(rel_key, path)
                name_key = path.name.lower()
                name_index.setdefault(name_key, []).append(path)

        self._factory_proxy_index_roots = roots
        self._factory_proxy_rel_index = rel_index
        self._factory_proxy_name_index = name_index

    def _factory_relative_candidates(self, path: Path) -> list[str]:
        candidates: list[str] = []
        ...
        raw_posix = str(path.as_posix())
        lowered = raw_posix.lower()
        for marker in ("/factory presets/", "/factory copy/"):
            idx = lowered.rfind(marker)
            if idx >= 0:
                add(raw_posix[idx + len(marker) :])

        try:
            add(path.resolve().relative_to(self.get_preset_dir().resolve()).as_posix())
        except Exception:  # noqa: BLE001
            pass

        try:
            add(path.resolve().relative_to(SYSTEM_FACTORY_PRESET_ROOT.resolve()).as_posix())
        except Exception:  # noqa: BLE001
            pass
    ...
    def _resolve_direct_factory_proxy_path(self, factory_path: Path) -> Path | None:
        self._refresh_factory_proxy_index()
        for rel_key in self._factory_relative_candidates(factory_path):
            direct = self._factory_proxy_rel_index.get(rel_key)
            if direct is not None:
                return direct

        candidates = self._factory_proxy_name_index.get(factory_path.name.lower(), [])
        if not candidates:
            return None

        def rank(path: Path) -> tuple[int, int]:
            source = path.as_posix().lower()
            preferred = 0 if "factory copy" in source else 1
            return preferred, len(source)

        candidates = sorted(candidates, key=rank)
        return candidates[0]

    def _load_direct_factory_proxy_presets(
        self,
        ...
        for factory_path in factory_paths:
            proxy_path = self._resolve_direct_factory_proxy_path(factory_path)
            if proxy_path is None:
                continue
            try:
                key = str(proxy_path.resolve())
            except OSError:
                key = str(proxy_path)
            ...
            if max_items is not None and len(presets) >= max_items:
                break
        return presets, mapped, skipped_invalid
    ...
    def _style_key(self, style_name: str) -> str:
        return style_name.strip().lower()

    def _path_style_tags(self, path: Path) -> set[str]:
        tokens = self._tokenize_text(path.stem)
        tokens.update(self._tokenize_text(path.parent.name))
        tags: set[str] = set()
    ...
    def _filter_paths_by_style(self, paths: list[Path], style_name: str) -> list[Path]:
        style_key = self._style_key(style_name)
        if style_key in ("", "auto"):
            return paths
        filtered = [path for path in paths if style_key in self._path_style_tags(path)]
        return filtered
    ...
    def _build_factory_proxy_pool(
        ...
        direct_presets, direct_mapped, direct_invalid = self._load_direct_factory_proxy_presets(
            factory_paths,
            max_items=max_pool,
        )
        ...
        if not editable_paths:
            if direct_presets:
                note = f"Direct factory mapping used {len(direct_presets)} converted preset(s)."
                if direct_invalid:
                    note += f" Skipped invalid direct matches: {direct_invalid}."
                return direct_presets[:max_pool], note
        ...
        if direct_presets:
            pool = self._merge_unique_presets(direct_presets, pool, max_items=max_pool)
            note = (
                f"Direct factory mapping found {len(direct_presets)} converted match(es) "
                f"from {direct_mapped}/{len(factory_paths)} source name(s). {note}"
            )
        elif direct_mapped:
            note = (
                f"Direct factory mapping found {direct_mapped} candidate path match(es) but none were valid editable presets. "
                f"{note}"
            )
    ...
    def run_mutate(self, refresh_list: bool = True, update_status: bool = True) -> list[Path]:
        outputs: list[Path] = []
        try:
            ...
            if self._is_zip_preset(source_path):
                ...
                direct_base_presets, _mapped_count, _invalid_count = self._load_direct_factory_proxy_presets(
                    [source_path],
                    max_items=1,
                )
                direct_base = direct_base_presets[0] if direct_base_presets else None
                proxy_pool, proxy_note = self._build_factory_proxy_pool([source_path], rng, max_pool=10)
                base_preset = direct_base or self._pick_exact_factory_proxy(source_path, proxy_pool)
                proxy_match_label = "direct-converted" if direct_base is not None else "exact"
    ...
    def run_combine(self, refresh_list: bool = True, update_status: bool = True) -> list[Path]:
        outputs: list[Path] = []
        try:
            ...
            if binary_paths:
                ...
                direct_proxy_presets, direct_proxy_mapped, direct_proxy_invalid = self._load_direct_factory_proxy_presets(
                    binary_paths,
                    max_items=len(binary_paths),
                )
                direct_by_resolved: dict[str, object] = {}
                for preset in direct_proxy_presets:
                    source_path = Path(getattr(preset, "source", ""))
                    try:
                        key = str(source_path.resolve())
                    except OSError:
                        key = str(source_path)
                    direct_by_resolved[key] = preset

                for path in binary_paths:
                    direct_path = self._resolve_direct_factory_proxy_path(path)
                    direct_preset = None
                    if direct_path is not None:
                        try:
                            direct_key = str(direct_path.resolve())
                        except OSError:
                            direct_key = str(direct_path)
                        direct_preset = direct_by_resolved.get(direct_key)
                    if direct_preset is not None:
                        source_proxy_pairs.append((path, direct_preset, 4, 999))
                        used_proxy_ids.add(id(direct_preset))
                        continue
                ...
                if len(source_proxy_pairs) >= 2:
                    ...
                    if direct_proxy_presets:
                        summary += (
                            f" Direct converted matches: {len(direct_proxy_presets)} "
                            f"(mapped paths: {direct_proxy_mapped}, invalid direct matches skipped: {direct_proxy_invalid})."
                        )
                    summary += f" {proxy_note}"
                    if use_full_folder:
    ...
    def run_random(self, refresh_list: bool = True, update_status: bool = True) -> list[Path]:
        outputs: list[Path] = []
        try:
            ...
            style_focus = self.random_style.get().strip() or "Auto"
            output_dir = self.get_output_dir()
            rng = random.Random(seed)
            use_guidance = bool(self.random_use_guidance.get())
            internal_presets, internal_skipped = self._load_internal_template_bank()
            if style_focus.lower() != "auto":
                internal_presets = [
                    preset
                    for preset in internal_presets
                    if self._style_key(style_focus) in self._path_style_tags(Path(getattr(preset, "source", "")))
                ]
            selected_paths = self._filter_paths_by_style(list(self.preset_paths), style_focus)
            if style_focus.lower() != "auto" and not selected_paths:
                selected_paths = list(self.preset_paths)
                notes_style_fallback = (
                    f"No presets matched style '{style_focus}' in selected folder; using full selected folder."
                )
            else:
                notes_style_fallback = ""
            ...
            def ensure_global_presets() -> None:
                nonlocal global_loaded, global_presets, global_skipped, skipped_invalid
                if global_loaded:
                    return
                global_loaded = True
                global_paths = self._discover_global_editable_paths(limit=1800)
                global_paths = self._filter_paths_by_style(global_paths, style_focus)
                if not global_paths:
                    global_presets = []
                    global_skipped = 0
                    return
                loaded, skipped = self._load_valid_editable_presets(global_paths)
                global_presets = loaded
                global_skipped = len(skipped)
                skipped_invalid += global_skipped
            ...
            if use_guidance and selected_binary_paths:
                direct_guidance, direct_guidance_mapped, direct_guidance_invalid = self._load_direct_factory_proxy_presets(
                    selected_binary_paths,
                    max_items=64,
                )
                if direct_guidance:
                    guidance_presets = self._merge_unique_presets(guidance_presets, direct_guidance, max_items=64)
                    notes.append(
                        f"Direct converted factory guidance: {len(direct_guidance)} "
                        f"(mapped: {direct_guidance_mapped}, invalid skipped: {direct_guidance_invalid})."
                    )
            ...
            if notes_style_fallback:
                notes.append(notes_style_fallback)
            if style_focus.lower() != "auto":
                notes.append(f"Style focus: {style_focus}.")

            if use_guidance:
                if len(guidance_presets) < 12:
                    ensure_global_presets()
                    if global_presets:
    ...
    def run_anomalize(self) -> None:
        ...
        try:
            ...
            for idx in range(count):
                self.status_var.set(f"Anomalize generating {idx + 1}/{count}...")
                ...
                for _attempt in range(5):
                    ...
                    if mode == "diffusion":
                        ...
                        created_now = self.run_mutate(refresh_list=False, update_status=False)
                    elif mode == "fusion":
                        ...
                        created_now = self.run_combine(refresh_list=False, update_status=False)
                    else:
                        ...
                        self._set_listbox_selection([])
                        created_now = self.run_random(refresh_list=False, update_status=False)
