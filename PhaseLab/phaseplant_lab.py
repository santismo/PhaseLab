...
BINARY_CANDIDATE_LIMIT_PER_LEN = 192
...
def _append_binary_candidate(store: Dict[int, List[str]], key_len: int, value: str) -> None:
    bucket = store.setdefault(key_len, [])
    if value in bucket:
        return
    if len(bucket) >= BINARY_CANDIDATE_LIMIT_PER_LEN:
        return
    bucket.append(value)


def _collect_binary_value_maps(
    ...
    donor_numeric_by_len: Dict[int, List[str]] = {}

    wavetable_lookup = {name.lower(): name for name in discover_wavetable_name_pool()}
    for donor in blobs:
        if not donor:
            continue
        for _start, _length, text, _has_null in _iter_length_prefixed_strings(donor):
            low = text.lower()
            if low in DEFAULT_ANALOG_WAVEFORMS:
                _append_binary_candidate(donor_waveforms_by_len, len(text), text)
            wavetable_name = wavetable_lookup.get(low)
            if wavetable_name:
                _append_binary_candidate(donor_wavetables_by_len, len(text), wavetable_name)
            if "." in text and "e" not in low and NUMERIC_RE.fullmatch(text):
                _append_binary_candidate(donor_numeric_by_len, len(text), text)
    return donor_waveforms_by_len, donor_wavetables_by_len, donor_numeric_by_len


def _safe_float_value(raw: bytes, offset: int) -> float | None:
    if offset < 0 or offset + 4 > len(raw):
        return None
    value = struct.unpack_from("<f", raw, offset)[0]
    if not math.isfinite(value):
        return None
    if abs(value) > 50_000.0:
        return None
    return float(value)


def _collect_binary_float_offsets(base_blob: bytes, donor_blobs: Sequence[bytes], max_offsets: int = 256) -> List[int]:
    if not donor_blobs:
        return []
    offsets: List[int] = []
    upper = max(0, len(base_blob) - 4)
    for offset in range(0, upper + 1, 4):
        base_value = _safe_float_value(base_blob, offset)
        if base_value is None:
            continue
        # Skip exact-zero heavy fields; low yield and higher risk of touching flags.
        if abs(base_value) < 1e-8:
            continue
        found = False
        for donor in donor_blobs:
            donor_value = _safe_float_value(donor, offset)
            if donor_value is None:
                continue
            delta = abs(base_value - donor_value)
            if delta < 1e-6 or delta > 30_000.0:
                continue
            found = True
            break
        if not found:
...
def mutate_binary_preset_blob(
    ...
    data = bytearray(blob)
    changed = 0
    donors = [item for item in (donor_blobs or ()) if item]

    wavetable_pool = discover_wavetable_name_pool()
    waveform_pool = DEFAULT_ANALOG_WAVEFORMS
    donor_waveforms_by_len, donor_wavetables_by_len, donor_numeric_by_len = _collect_binary_value_maps(
        donors
    )
    ...
    for start, length, text, has_null in list(_iter_length_prefixed_strings(bytes(data))):
        ...
        elif "." in text and "e" not in low and NUMERIC_RE.fullmatch(text) and rng.random() < min(1.0, 0.22 + 0.55 * amount):
            replacement = _same_length_replacement(text, donor_numeric_by_len.get(len(text), []), rng)
            if replacement is None:
    ...
    # Conservative donor-guided float mutation for binaries with sparse string fields.
    candidate_offsets = list(float_offsets) if float_offsets else _collect_binary_float_offsets(bytes(data), donors)
    if candidate_offsets and (changed == 0 or rng.random() < (0.20 + 0.65 * amount)):
        edits = min(len(candidate_offsets), max(1, int(round(1 + amount * 6))))
        for offset in rng.sample(candidate_offsets, edits):
            base_value = _safe_float_value(bytes(data), offset)
            if base_value is None:
                continue
            ...
            for donor in donors:
                ...
                if donor_value is None:
                    continue
                if abs(base_value - donor_value) < 1e-6:
                    continue
                donor_values.append(donor_value)
            if not donor_values:
                continue
            target = rng.choice(donor_values)
            blend = 0.25 + 0.55 * amount
            new_value = base_value + (target - base_value) * blend
            jitter = (rng.random() * 2.0 - 1.0) * max(1e-4, abs(new_value) * 0.03 * amount)
            new_value += jitter
            if not math.isfinite(new_value):
                continue
            if abs(new_value) > 50_000.0:
                continue
            try:
                packed = struct.pack("<f", float(new_value))
            except (OverflowError, struct.error):
                continue
            if packed == bytes(data[offset : offset + 4]):
                continue
...
def combine_binary_preset_blobs(
    ...
    donor_waveforms_by_len, donor_wavetables_by_len, donor_numeric_by_len = _collect_binary_value_maps(donors)
    ...
    for start, length, text, has_null in base_slots:
        ...
        if low in waveform_pool:
            ...
            replacement = _same_length_replacement(text, donor_wavetables_by_len.get(len(text), []), rng)
        elif "." in text and "e" not in low and NUMERIC_RE.fullmatch(text):
            replacement = _same_length_replacement(text, donor_numeric_by_len.get(len(text), []), rng)
        if replacement is None:
            continue
        encoded = replacement.encode("utf-8")
    ...
    candidate_offsets = list(float_offsets) if float_offsets else _collect_binary_float_offsets(bytes(data), donors)
    if candidate_offsets and mix_rate > 0.0:
        edits = min(len(candidate_offsets), max(1, int(round(1 + mix_rate * 7))))
        for offset in rng.sample(candidate_offsets, edits):
            base_value = _safe_float_value(bytes(data), offset)
            if base_value is None:
                continue
